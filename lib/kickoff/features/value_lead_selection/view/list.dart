...
import 'package:mevn_app/kickoff/features/value_lead_selection/bloc/value_lead_selection_bloc.dart';
...
class ListValueLeadSelectionScreen extends StatefulWidget {
  const ListValueLeadSelectionScreen({Key? key}) : super(key: key);

  @override
  State<ListValueLeadSelectionScreen> createState() => _ListMemberState();
}

class _ListMemberState extends State<ListValueLeadSelectionScreen> {
  late ValueLeadSelectionBloc? _valueLeadSelectionBloc;
  late List<String> pms = [];
  late bool isShow = true;
  final ValueLeadSelection _valueLeadSelection = ValueLeadSelection();
  int maxSelected = 5;
  @override
  void initState() {
    super.initState();
    _valueLeadSelectionBloc = ValueLeadSelectionBloc(
      locator.get(),
      locator.get(),
      locator.get(),
    );
  ...
  Widget build(BuildContext context) {
    ...
    return ContentWidget(
      ...
      body: BlocProvider(
        create: (_) => _valueLeadSelectionBloc!..getListMember(),
        child: BlocBuilder<ValueLeadSelectionBloc, ValueLeadSelectionState>(
          builder: (context, state) {
            if (state is LoadingState) {
              ...
              if (state is ValueIsCheckedState) {
                pms = state.data ?? [];
                            ...
                            ? ElevatedButton(
                                ...
                                child: Text(
                                  pms.length < maxSelected
                                      ? 'Please select ${maxSelected - pms.length} more Value Lead'
                                      : 'Send',
                                ),
                              )
                            : const Text(
              ...
              } else if (state is ErrorValueLeadSelectionState) {
                return Center(
                  child: Text(
                    state.errorMessage,
  ...
  void submitData(BuildContext context) {
    if (pms.length == maxSelected) {
      setState(() {
        _valueLeadSelection.pms = pms;
      });
      _valueLeadSelectionBloc!.sendSelection(_valueLeadSelection);
    } else {
      GlobalSnackBar.show(context, 'Please select Value Lead!', 'danger');
    }
  }
}
